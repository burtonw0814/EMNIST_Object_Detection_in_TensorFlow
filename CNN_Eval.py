...
def get_view(im_ready, group_l1, group_l2, group_l3, group_l4, threshold, p1, p2, pw, ph, num_grid_cells1, num_grid_cells2, num_anchor_boxes, num_classes):
    class_mapping = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabdefghnqrt'

    box_pad=2
    im_demo=[]
    im_demo=np.array(im_ready)
    ht=im_ready.shape[0]
    wd=im_ready.shape[1]
    box_color_triplets=((255,0,0),(0,255,0))
    t=1
    
    # Processes predicted feature map into boundig box and displays onto image
    # Indices in l_3 that exceed thresh
    boxes=[];
    conf=[];
    for ijij in range(len(group_l1)): # Iterate over scales
        #for kjkj in  range(len(num_grid_cells1)): # Iterate over anchor boxes
        l1=group_l1[ijij]
        l2=group_l2[ijij]
        l3=group_l3[ijij]
        l4=group_l4[ijij]
        for l in range(l3.shape[-1]): # num anchor boxes 
            idx=np.where(l3[0,:,:,l]>threshold)
            #print(idx)

            for k in range(len(idx[0])):
                #print()
                tx=l1[0,idx[0][k],idx[1][k],0+(2*l)]
                ty=l1[0,idx[0][k],idx[1][k],1+(2*l)]
                tw=l2[0,idx[0][k],idx[1][k],0+(2*l)]
                th=l2[0,idx[0][k],idx[1][k],1+(2*l)]
                to=l3[0,idx[0][k],idx[1][k],l]
                c=l4[0,idx[0][k],idx[1][k],num_classes*l:num_classes*(l+1)]
                #print(tx, ty, tw, th, to)

                #print(len(num_grid_cells2), num_grid_cells2)
                #print(idx, type(idx), len(idx), idx[0])

                bx=int(p2/num_grid_cells2[ijij])*(tx+idx[1][k])# Center x --> bx is in units of grid cell coords so multiply by number of pixels per grid cell
                by=int(p1/num_grid_cells1[ijij])*(ty+idx[0][k]) # Center y
                bw=pw[ijij][l]*np.exp(tw)*pc2
                bh=ph[ijij][l]*np.exp(th)*pc1

                # Get box corners --> Currently not robust to edge cases
                v1=np.amax((3,int(by-0.5*bh)-box_pad))
                v2=np.amin((int(by+0.5*bh)+box_pad,ht-3))
                h1=np.amax((3,int(bx-0.5*bw)-box_pad))
                h2=np.amin((int(bx+0.5*bw)+box_pad,wd-3))
                #print(v1,v2,h1,h2)

                temp=(np.float64(h1)/np.float64(wd), np.float64(h2)/np.float64(wd), np.float64(v1)/np.float64(ht), np.float64(v2)/np.float64(ht), np.argmax(c))
                #print(c, temp[-1])
                boxes.append(temp) # Keep list of box corners in units of percentage of image
                conf.append(to)
    ...
    while boxes:
        ...
        B_O_I=np.where(conf==np.amax(conf))[0][0]
        new_b.append(boxes[B_O_I])
        new_c.append(conf[B_O_I])
        
        for k in range(len(boxes)):
            # Get IOU
            IOU_mat=np.zeros((100,100,2))
            IOU_mat[int(100*boxes[B_O_I][0]):int(100*boxes[B_O_I][1]),int(100*boxes[B_O_I][2]):int(100*boxes[B_O_I][3]),0]=1
            IOU_mat[int(100*boxes[k][0]):int(100*boxes[k][1]),int(100*boxes[k][2]):int(100*boxes[k][3]),1]=1
            intersection=np.sum(IOU_mat[:,:,0]*IOU_mat[:,:,1])
            union=np.sum(np.amax(IOU_mat,axis=-1))
            IOU=intersection/union
            #print(IOU)
            if IOU<0.7: # If IOU is low enough, keep box
                filtered_b.append(boxes[k])
                filtered_c.append(conf[k])
                
        boxes=filtered_b
        conf=filtered_c   
    boxes=new_b
    
    # Create bounding box based on filtered boxes
    for k in range(len(boxes)):
        v1=int(ht*boxes[k][2])
        v2=int(ht*boxes[k][3])
        h1=int(wd*boxes[k][0])
        ...
        im_demo[v1:v2, h2-t:h2+t, :]=box_color_triplets[0]#[boxes[k][-1]]
        im_demo[v1-t:v1+t, h1:h2, :]=box_color_triplets[0]#[boxes[k][-1]]
        im_demo[v2-t:v2+t, h1:h2, :]=box_color_triplets[0]#[boxes[k][-1]]
        im_demo[v1:v2, h1-t:h1+t, :]=box_color_triplets[0]

    # Now add class labels onto image
    im_obj=Image.fromarray(im_demo)
    ...
    for k in range(len(boxes)):
        ...
        # Draw class label onto image
        draw.text((h1,v1),class_mapping[boxes[k][-1]],(0,255,0))
    im_demo=np.array(im_obj)

    return im_demo, boxes
...
image_file='Images_Test/'
box_file='Annotations_Simple_Test/' 
...
with tf.Session() as sess:
    ...
    saver.restore(sess, './Model/Scratch_722500_')
